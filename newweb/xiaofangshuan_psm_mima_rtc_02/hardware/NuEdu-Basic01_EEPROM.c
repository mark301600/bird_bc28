...
#define I2C_EEPROM_IRQn I2C0_IRQn
...
/*  Write I2C                                                                                        */
/*---------------------------------------------------------------------------------------------------------*/
void EEPROM_Write(uint32_t u16Address, char *DataBuffer,uint32_t count)
{
	uint32_t i;
	char *buf;
	buf=DataBuffer;
	for(i=0;i<count;i++)
	{
		I2C_EEPROM_Write(u16Address,*buf);
//		*buf=I2C_EEPROM_Read(u16Address);
		u16Address++;
		buf++;
	}
//	I2C_EEPROM_Write(u16Address,NULL);
}
/*---------------------------------------------------------------------------------------------------------*/
/*  Read I2C EEPROM                                                                                        */
/*---------------------------------------------------------------------------------------------------------*/
uint8_t EEPROM_Read(uint32_t u16Address, char *DataBuffer,uint32_t count)
{
	uint32_t i;
	char *buf;
	buf=DataBuffer;
